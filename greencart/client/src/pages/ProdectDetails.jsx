...
const ProdectDetails = () => {
    const { id } = useParams();
    const { products, currency, addToCart, cartItems, navigate } = useAppContext();
    ...
    if (!product) return <div className="mt-16 text-center text-gray-500">Product not found.</div>;
    ...
    return (
        <div className="max-w-6xl w-full px-6 mt-10">
            ...
            <div className="flex flex-col md:flex-row gap-16 mt-6">
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <ul className="list-disc ml-4 text-gray-500/70">
                        {(Array.isArray(product.description) ? product.description : (typeof product.description === 'string' ? product.description.split('\n') : [])).map((desc, index) => (
                            <li key={index}>{desc}</li>
                        ))}
                    </ul>

                    <div className="flex items-center mt-10 gap-4 text-base">
                        <button onClick={() => addToCart(product._id)} className="w-full py-3.5 cursor-pointer font-medium bg-primary/10 border border-primary/40 text-primary hover:bg-primary/20 transition">
                            Add to Cart {cartItems[product._id] ? `(${cartItems[product._id]})` : ''}
                        </button>
                        <button className="w-full py-3.5 cursor-pointer font-medium bg-primary text-white hover:bg-primary/90 transition">
                            Buy now
                        </button>
                    </div>
                </div>
            ...
            <div className="flex flex-col items-center mt-20">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mt-6 w-full">
                    {products.filter((p) => p.category === product.category && p._id !== id).map((p, index) => (
                        <ProductCard key={index} product={p} />
                    ))}
                </div>
                <button
                    onClick={() => { navigate(`/products/${product.category.toLowerCase()}`); scrollTo(0, 0); }}
                    className="mt-8 px-10 py-2.5 border border-primary text-primary rounded hover:bg-primary hover:text-white transition"
                >
                    See more
                </button>
            </div>
